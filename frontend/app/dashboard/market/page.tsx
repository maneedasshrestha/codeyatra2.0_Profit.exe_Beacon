...
import { MarketItem, Role, AddItemForm } from "./components/types";
...
const SEED_ITEMS: MarketItem[] = [
  {
    ...
    description: "Classic instant film camera in mint condition. Great for aesthetic shots.",
    price: 1499,
    colorSeed: 0,
  ...
  },
  {
    ...
    description: "Hand-poured soy wax candles with real lavender. Burns 40+ hrs each.",
    price: 599,
    colorSeed: 1,
  ...
  },
  {
    ...
    description: "75% layout, hot-swappable, clicky switches. Purple & pink keycaps.",
    price: 3299,
    colorSeed: 7,
...
export default function MarketPage() {
  ...
  const visibleItems = role === "buyer" && search.trim()
    ? items.filter((it) =>
  ...
  const handleDelete = (id: string) => {
    ...
    setNewlyListed((prev) => { const s = new Set(prev); s.delete(id); return s; });
  ...
  const handleRoleSwitch = (next: Role) => {
    ...
    if (next === "buyer" && newlyListed.size > 0) {
      showToast(`${newlyListed.size} new item${newlyListed.size > 1 ? "s" : ""} listed in the marketplace`);
    }
  };
  ...
  const handleBuyConfirm = () => {
    ...
    setNewlyListed((prev) => { const s = new Set(prev); s.delete(buyTarget.id); return s; });
    setWishlistIds((prev) => { const s = new Set(prev); s.delete(buyTarget.id); return s; });
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(160deg,#f4f4f6 0%,#ececf0 40%,#f7f7f9 100%)",
      }}
    >
      {/* Decorative blobs */}
      <div
        className="pointer-events-none fixed top-[-80px] left-[-60px] w-64 h-64 rounded-full opacity-40"
        style={{ background: "radial-gradient(circle,#c084fc55,transparent 70%)", zIndex: 0 }}
      />
      <div
        className="pointer-events-none fixed bottom-24 right-[-50px] w-52 h-52 rounded-full opacity-30"
        style={{ background: "radial-gradient(circle,#818cf855,transparent 70%)", zIndex: 0 }}
      />

      {/* ── Content shell ─────────────────────────────────────────────────── */}
      <div className="relative z-10 w-full max-w-[412px] flex flex-col h-screen">
          ...
          <div className="flex items-start justify-between mb-3">
            ...
            {role === "buyer" && (
              <div className="flex items-center gap-2 mt-1">
                ...
                <button
                  onClick={() => { setShowWishlist((v) => !v); setShowPurchases(false); }}
                  className="relative flex flex-col items-center gap-0.5 w-12 h-12 rounded-2xl items-center justify-center active:scale-95 transition-transform"
                  style={{
                    ...
                    color: showWishlist ? "#fff" : wishlistIds.size > 0 ? "#7c3aed" : "#9ca3af",
                    boxShadow: wishlistIds.size > 0 ? "0 4px 16px rgba(124,58,237,0.25)" : "none",
                    display: "flex",
                  }}
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill={showWishlist || wishlistIds.size > 0 ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                {purchased.length > 0 && (
                  <button
                    onClick={() => { setShowPurchases((v) => !v); setShowWishlist(false); }}
                  ...
                  >
                    ...
                    <span className="text-xs font-extrabold">{purchased.length}</span>
                  </button>
                )}
              </div>
            )}
        ...
        {/* ── Scrollable body ────────────────────────────────────────────── */}
        <div className="flex-1 overflow-y-auto px-4 pt-3 pb-32" style={{ scrollbarWidth: "none" }}>
          ...
          {showWishlist && role === "buyer" && (
            ...
            >
              <p className="text-sm font-extrabold text-violet-800 mb-3 flex items-center gap-2">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: "#7c3aed" }}>
                  <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
                </svg>
                Saved Items ({wishlistIds.size})
              </p>
              {wishlistedItems.length === 0 ? (
                <p className="text-xs text-gray-400 text-center py-3">Tap the heart on any item to save it here.</p>
              ) : (
                <div className="flex flex-col gap-2">
                  {wishlistedItems.map((it) => (
                    <div
                      ...
                      onClick={() => { handleViewDetail(it); setShowWishlist(false); }}
                    >
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-gray-800 truncate">{it.name}</p>
                        <p className="text-[11px] text-gray-400 truncate">{it.sellerName} · {it.location}</p>
                      </div>
                      <div className="flex items-center gap-2 ml-3 shrink-0">
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); handleWishlist(it); }}
